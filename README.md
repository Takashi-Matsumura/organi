...
組織構造を視覚的に管理し、社員の異動をドラッグ&ドロップで直感的に操作できるWebアプリケーションです。人事評価システムとの連携も可能で、組織図を超えた柔軟な評価関係管理に対応しています。APIキー認証システムにより、権限に応じたアクセス制御を実現しています。
...
- **3段階の権限管理**:
  ...
  - **VIEWER（閲覧者権限）**: 閲覧のみ可能
- **デモキー提供**: 各権限レベルのテスト用APIキーを内蔵
- **ローカルストレージ**: ブラウザにAPIキーを安全に保存
...
│   ├── ApiKeyInput.tsx           # APIキー入力・認証コンポーネント
...
#### デモ用APIキー
アプリケーション起動時に以下のデモキーでログインできます：

- **管理者権限**: `demo-admin-key-2024`
  - 全機能にアクセス可能（閲覧・編集・削除）
- **編集者権限**: `demo-editor-key-2024`
  - 閲覧・編集が可能（削除機能は無効）
- **閲覧者権限**: `demo-viewer-key-2024`
  - 閲覧のみ可能（編集・削除機能は無効）

#### ログイン手順
1. アプリケーションにアクセス
2. 中央のログインモーダルでAPIキーを入力
3. 「認証」ボタンをクリック
...
### 2025年9月9日更新
- **APIキー認証システム**: 3段階の権限管理（ADMIN・EDITOR・VIEWER）を実装
- **ログインモーダル**: 美しいログイン画面でユーザー体験を向上
- **権限制御**: 各権限レベルに応じた機能アクセス制御
- **デモキー**: テスト用のAPIキーを内蔵して即座にテスト可能
