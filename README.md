...
### 2025年9月15日更新 - UIレイアウト最適化
- **ヘッダーレイアウト最適化**: コンテキスト固有のコントロールを適切な場所に移動
  - 「エクスポート」「インポート」ボタンはグローバルヘッダーに維持
  - 「編集モード」ボタンを組織図タブに移動（コンテキスト固有機能）
  - 「APIテスト」ボタン（管理者限定）を組織図タブに移動
  - ボタンにアイコン+テキスト表示を追加してユーザビリティを向上
- **コンテキスト指向UI**: 機能が必要な場所に配置することでユーザー体験を改善
